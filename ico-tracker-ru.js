...
var app = new Vue({
  ...
  computed: {
    progress: function() {
      ...
      if (!this.totalSupply || !this.ethPrice) return 'Загрузка...';
      ...
      return '[' + temp + '] ' + percentage.toFixed(2) + '% до $250,000';
    },
    raised: function() {
      if (this.error) return 'Пожалуйста, перезагрузите страницу, чтобы увидеть прогресс ICO';
      if (!this.totalSupply || !this.ethPrice) return '';

      return 'Собрано: ' + (this.totalSupply / 250) + ' ETH ($' + (this.totalSupply / 250 * this.ethPrice).toFixed(2) + ' или ' + this.totalSupply + ' FDU)'
    }
  },
  created: function() {
    try {
